...
def main_func(input):
    answers = []
    for i in input:
        hyphen_position = i.index('-')
        colon_position = i.index(':')
        
        start_value = int(i[0:hyphen_position])
        end_value = int(i[hyphen_position + 1: colon_position - 2])
        letter = i[colon_position - 1]
        string_search = i[colon_position + 2:]

        
        if string_search[start_value - 1] == letter and string_search[end_value - 1] != letter:
            answers.append(i)
        elif string_search[start_value - 1] != letter and string_search[end_value - 1] == letter:
            answers.append(i)
