...
def check_valid(inputz):
    answer = inputz[4:]
    if inputz[0:3] == 'byr':
        if 1920 <= int(answer) <= 2002:
            return True
    if inputz[0:3] == 'iyr':
        if 2010 <= int(answer) <= 2020:
            return True
    if inputz[0:3] == 'eyr':
        if 2020 <= int(answer) <= 2030:
            return True
    if inputz[0:3] == 'hgt':
        if len(inputz[4:]) >= 4:
            units = inputz[len(inputz)-2:]
            numbers = int(inputz[4:len(inputz)-2])
            if units == "cm":
                if 150 <= numbers <= 193:
                    return True
            elif units == "in":
                if 59 <= numbers <= 76:
                    return True
    if inputz[0:3] == 'hcl':
        if inputz[4] == '#' and inputz[5:].isalnum():
            return True
    if inputz[0:3] == 'ecl':
        if inputz[4:] in ['amb','blu','brn','gry','grn','hzl','oth']:
            return True
    if inputz[0:3] == 'pid':
        try:
            if int(inputz[4:]) and len(inputz[4:]) == 9:
                return True
        except:
            return None
    if inputz[0:3] == 'cid':
...
acceptable_combos = [['byr','cid','ecl','eyr', 'hcl','hgt','iyr','pid'],
...
for i in b:
    ...
    for j in range(0,len(i)):
        if i[j] == ':':
            codes.append(i[j-3:j])
        codes.sort()
    just_codes.append(codes)

for i in b:
    j = i.replace('\n',' ')
    codes= make_list(j," ")
    codes.sort()
    all_answers.append(codes)


counter = 0
for i in range(0,len(just_codes)):
    if just_codes[i] in acceptable_combos:
        countme = 0
        for j in all_answers[i]:
            if check_valid(j) == None:
                countme += 1
        if countme == 0:
            counter += 1
