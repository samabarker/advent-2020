...
check_against = 'abcdefghijklmnopqrstuvwxyz'
input_list = make_list(input_str)
input_list_corrected = []
answers = []
num_of_people = []
...
for i in input_list:
    new = i.replace('\n', ' ')
    input_list_corrected.append(new)

for i in input_list_corrected:
    counter = 1
    for j in i:
        if j == ' ':
            counter += 1
    num_of_people.append(counter)

for i in range(0,len(input_list_corrected)):
    counter2 = 0
    for j in check_against:
        counter = 0
        for k in input_list_corrected[i]:
            if k == j:
                counter += 1
        if counter == num_of_people[i]:
            counter2 += 1
    answers.append(counter2)
