inputz = '''pale cyan bags contain 2 posh black bags, 4 wavy gold bags, 2 vibrant brown bags.
...
def make_list(inputz, splitz = '\n'):
    order = list(inputz.split(splitz))
    return order

list_of_statements = make_list(inputz)
list_of_bags = []
list_of_components = []
refs_outer = []
bags_in_bags = []
...
for i in list_of_statements:
    cut_off = i.index('contain')
    list_of_bags.append((i[:cut_off-6]).replace(' ',''))
...
for i in list_of_statements:
    ...
    for j in i:
      if j.isnumeric():
        counter += int(j)
    list_of_components.append(counter)
...
for i in list_of_statements:
    ...
    for j in range(0,len(i)):
      if i[j].isnumeric() or i[j] == "." or i[j] == ",":
        refs.append(j)
    refs_outer.append(refs)

for i in range(0,len(list_of_statements)):
    new_list = []
    new_list2 = []
    if len(refs_outer[i])%2 == 0:
      for j in range(0,int(len(refs_outer[i])/2)):
        new_list2.append(list_of_statements[i][refs_outer[i][j*2]])
        new_list.append((list_of_statements[i][refs_outer[i][j*2]+2:refs_outer[i][j*2+1]-4]).replace(' ',''))
    bags_in_bags.append(new_list)
    bags_in_bags_num.append(new_list2)


total = 0

def check_for_gold(list_bags, list_multis = [1], multi = ''):
    global total
    for i in range(0,len(list_bags)):
        multi = multi + str(list_multis[i])
        this_total=1
        #print(list_bags)
        #print(list_multis)
        #print(multi)
        for j in multi:
          this_total = this_total * int(j)
        total = total + this_total
        current_bag = list_bags[i]
        bag_ref = list_of_bags.index(current_bag)
        new_list = bags_in_bags[bag_ref]
        new_list_nos = bags_in_bags_num[bag_ref]
        if len(new_list) > 0:
          check_for_gold(new_list, new_list_nos, multi)
        multi = multi[:-1]

check_for_gold(['shinygold'])
print(total-1)
